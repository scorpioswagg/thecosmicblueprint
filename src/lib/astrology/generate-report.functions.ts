...
export const generateAstroReport = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: rows, error: catalogError } = await context.supabase
      .from("report_catalog")
    ...
    const report = mergeCatalog((rows ?? []) as unknown as CatalogRow[], { includeInactive: true }).find(
    ...
    if (report.accessMode === "paid") {
      ...
      if (!purchase) {
        throw new Error("PAYMENT_REQUIRED: Please purchase this report to unlock generation. Administrator accounts receive free access automatically.");
      }
    }
    ...
    if (report.adult) {
      ...
      if (!profile?.adult_consent) {
        throw new Error("ADULT_CONSENT_REQUIRED: You must record 18+ consent before generating intimacy reports.");
      }
    }
