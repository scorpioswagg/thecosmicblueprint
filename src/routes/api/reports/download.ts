...
function adminDb() { const url = process.env.SUPABASE_URL; const key = process.env.SUPABASE_SERVICE_ROLE_KEY; if (!url || !key) throw new Error("Supabase server credentials are not configured"); return createClient(url, key, { auth: { persistSession: false, autoRefreshToken: false } }); }
...
export const Route = createFileRoute("/api/reports/download")({
  server: { handlers: { GET: async ({ request }) => {
    ...
    } catch (error) { console.error("[report-download] failed", error); return new Response("Download failed", { status: 500 }); }
  } } },
});
